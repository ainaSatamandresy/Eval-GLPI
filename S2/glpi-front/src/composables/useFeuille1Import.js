...
const TYPE_MAP = {
    'Incident': 1, 'Demande': 2, 'ticket':1,  // pour tickets
...
export function useFeuille1Import() {
    ...
    async function importFile(file) {
        ...
        for (let i = 0; i < rows.length; i++) {
            const row = rows[i]
            const rawItemType = row['Item_Type']?.trim().toLowCase()
            const itemtype = TYPE_MAP['Item_Type'] || TYPE_MAP[rawItemType] || 'Computer' // Par défaut Computer si non reconnu
