          ...
          <table class="data-table">
            ...
            <tbody>
              ...
              <tr v-if="tickets.length === 0">
                <td colspan="4" class="empty-row">Aucun ticket trouvé.</td>
              </tr>
            </tbody>
          </table>
        </div>
...
const ticketCosts = ref({}) // mapping ticket_id => cost
...
async function loadData() {
  ...
  try {
    ...
    const initCosts = {}
    fetchedTickets.forEach(t => { initCosts[t.id] = 0 })
    ...
    try {
      ...
      fetchedTickets.forEach(t => {
        ...
        matchingCosts.forEach(c => {
          sum += (Number(c.cost_time) || 0) + (Number(c.cost_fixed) || 0)
        })
        if (sum > 0) initCosts[t.id] = parseFloat(sum.toFixed(2))
      })
    } catch (e) {
      // ignorer si l'API n'a pas cette entité
    }
